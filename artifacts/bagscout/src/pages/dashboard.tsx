...
import { ArrowRight, Bell, Bookmark, Search, ShieldCheck } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const { data: recentMatches, isLoading: isMatchesLoading } = useGetRecentMatches(
    { limit: 4 },
    { query: { queryKey: getGetRecentMatchesQueryKey({ limit: 4 }) } },
  );
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="rounded-none border-border shadow-none">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-serif font-medium">
              {summary?.activePreferenceCount ?? 0}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              of {summary?.preferenceCount ?? 0} total
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-none border-border shadow-none">
          <CardContent className="p-6">
            <div className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Bell
                className={`h-4 w-4 ${
                  summary?.unreadAlertCount ? "text-primary" : "text-muted-foreground"
                }`}
              />
            </div>
        ...
        <Card className="rounded-none border-border shadow-none">
          <CardContent className="p-6">
            ...
            <Link
              href="/saved"
              className="text-xs text-muted-foreground hover:text-foreground mt-1 inline-block transition-colors"
            >
              View saved
            </Link>
          </CardContent>
                ...
                <div className="space-y-4">
                  {summary.topBrands.map((brand, i) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold bg-secondary px-2 py-1">
                        {brand.count}
                      </span>
                    </motion.div>
                  ))}
          ...
          <Card className="rounded-none border-border shadow-none bg-primary/5 border-primary/20">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground leading-relaxed">
                For the highest chance of securing a sought-after piece, ensure your{" "}
                <strong>max price</strong> reflects current market reality. Items priced
                significantly below market are often in poor condition or sell within minutes.
              </p>
            </CardContent>
          </Card>
        </div>
