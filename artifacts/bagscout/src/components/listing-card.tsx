      ...
      <Card className="overflow-hidden rounded-none border border-border group hover:border-primary/50 transition-all duration-300 h-full flex flex-col bg-card">
        <div className="relative aspect-square overflow-hidden bg-secondary/30">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            <Badge className={`rounded-none font-semibold ${getSourceColor(listing.sourceName)} border-none shadow-sm`}>
              <ShieldCheck className="w-3 h-3 mr-1" />
              {listing.sourceName}
            </Badge>
            
            {matchScore !== undefined && (
              <Badge variant="outline" className="rounded-none bg-background/95 backdrop-blur-sm border-primary/20 text-primary font-bold shadow-sm self-start">
        ...
        <CardContent className="p-4 flex-1 flex flex-col">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h3 className="font-bold text-sm tracking-widest uppercase text-muted-foreground">{listing.brand}</h3>
          ...
          <div className="mt-auto pt-4 flex flex-col gap-3">
            {matchReasons && matchReasons.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {matchReasons.slice(0, 3).map((reason, i) => (
                  <span key={i} className="text-[10px] uppercase tracking-wider bg-secondary px-1.5 py-0.5 text-secondary-foreground">
                    {reason}
                  </span>
                ))}
            ...
            <div className="flex justify-between items-end">
              ...
              <Badge variant="outline" className="rounded-none border-border text-xs uppercase tracking-wider font-normal">
                {listing.condition}
              </Badge>
            </div>
